...
def invoke_react_agent(response_schema: type[BaseModel], messages: List[BaseMessage], llm, tools) -> tuple[Dict[str, Any], List[str]]:
    ...
    tools_used = [t.name for t in result.get("messages", []) if isinstance(t, ToolMessage) and t.name is not None]
...
def classify_intent(state: AgentState, config: RunnableConfig) -> dict:
    ...
    llm = cast(ChatOpenAI, config.get("configurable").get("llm"))
    ...
    prompt = cast(PromptTemplate, get_intent_classification_prompt())

    chain = prompt | structured_llm
    intent = cast(UserIntent, chain.invoke({"user_input": state["user_input"], "conversation_history": state["messages"]}))
    ...
    if intent.intent_type == "qa":
        ...
        raise Exception("Unknown intent from user")
...
def summarization_agent(state: AgentState, config: RunnableConfig) -> dict:
    ...
    prompt_template = get_chat_prompt_template("summarize")
...
def update_memory(state: AgentState, config: RunnableConfig) -> dict:
    ...
    structured_llm = llm.with_structured_output(
        UpdateMemoryResponse # TODO Pass in the correct schema from scheams.py to extract conversation summary, active documents
    )

    response = cast(UpdateMemoryResponse, structured_llm.invoke(prompt_with_history))
    return {
